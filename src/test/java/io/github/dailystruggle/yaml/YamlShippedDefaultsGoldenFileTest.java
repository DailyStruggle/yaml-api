 ...
 * {@code YAML-plugin/src/main/resources/} must parse without error and
 ...
 */
class YamlShippedDefaultsGoldenFileTest {

    private static Path shippedRoot() {
        // YAML-core tests run with project root as CWD via Gradle.
        ...
        // If the test runs from YAML-core/, go up one level.
    ...
    Stream<DynamicTest> everyShippedYamlIsIdempotent() throws IOException {
        ...
        try (Stream<Path> walk = Files.walk(root)) {
            walk.filter(p -> p.toString().endsWith(".yml"))
                // plugin.yml is the Bukkit plugin manifest, parsed by the server (not by YAML's
    ...
    private static void assertIdempotent(Path file) throws IOException {
        ...
        YamlMapping once;
        try {
            once = YamlReader.parse(src);
        } catch (YamlParseException e) {
            fail("Parse failed for " + file + " — " + e.getMessage()
                    + " (messageKey=" + e.messageKey() + ")");
            return;
        }
        String emitted = YamlWriter.emit(once);
        YamlMapping twice = YamlReader.parse(emitted);
        String reemitted = YamlWriter.emit(twice);
